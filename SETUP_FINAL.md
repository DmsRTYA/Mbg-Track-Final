...
```sql
CREATE DATABASE mbg_track CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
```
...
> PENTING: Gunakan `npm run dev` (bukan `next dev`) karena server custom
> diperlukan untuk WebSocket. File entry point adalah `server.js`.
...
### Railway (MySQL + Next.js)
1. Buat MySQL di Railway → copy DATABASE_URL
2. Push ke GitHub → import di Railway
3. Set environment variables (DATABASE_URL, VAPID_*, dll)
4. Railway otomatis jalankan `npm run dev` (atau `npm start`)

### Vercel + PlanetScale
1. Database di PlanetScale → copy DATABASE_URL
2. Deploy ke Vercel → set env vars
3. **Catatan**: Vercel serverless tidak support WebSocket long-lived.
   Gunakan Railway atau VPS untuk WebSocket penuh.

### VPS (recommended untuk WebSocket)
```bash
npm install
npm run db:migrate:deploy
npm run db:seed
npm run build
npm start        # production mode dengan WebSocket
```
